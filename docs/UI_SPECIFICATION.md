...
**重要**: 会計年度設定は全画面の月表示に連動し、顧客予算CSV取り込み時に自動検出される。
...
| generateFiscalMonthIndices() | 月インデックス配列生成（例: 4月開始→[3,4,5,...,0,1,2]） |
| autoDetectFiscalYearFromCSV() | CSV読込時に月情報から会計年度を自動検出・適用 |

### 23.10 CSV自動検出機能

顧客予算CSVを取り込む際に、月列情報から会計年度を自動検出して設定する機能。

#### 検出ロジック
1. CSVの月列ヘッダーを解析
2. 最初の月から開始月を検出（例: "4月" → startMonth = 4）
3. ヘッダー行に年情報があれば年度を検出（例: "2026年4月" → year = 2026）
4. fiscalYearSettings を更新
5. FISCAL_ORDER を再生成
6. プレビュー表示を更新
7. 自動検出情報をUI上に表示
...
会計年度設定はアプリ内の全月表示部分に自動連動します。

#### 連動箇所一覧

| 画面・機能 | 月表示箇所 |
|-----------|-----------|
| MQシミュレーション | 結果テーブルヘッダー |
| 予算一覧 | テーブルヘッダー |
| 客単価設定 | 月選択プルダウン、テーブルヘッダー |
| マスタ履歴プレビュー | 月別テーブルヘッダー |
| マスタ履歴編集 | 月別入力フォームヘッダー |
| シミュレーション入力 | 月選択プルダウン、入力テーブルヘッダー |
| 比較レポート | テーブルヘッダー |
| テンプレートダウンロード | CSV月列ヘッダー |

#### 技術仕様
- 全ての月表示は `FISCAL_ORDER` グローバル変数を参照
- 会計年度設定変更時は `generateFiscalOrder()` で `FISCAL_ORDER` を再生成
- 各テーブル・UI描画関数は `FISCAL_ORDER` を直接参照
...
│ 🏪 対象店舗選択                                              │
├─────────────────────────────────────────────────────────────┤
│ 設定対象店舗: [🌐 全店舗共通（デフォルト）▼] [📋 複数店舗一括設定]│
├─────────────────────────────────────────────────────────────┤
│ 📊 店舗別会計年度設定一覧:                                    │
│ ┌─────────────────────────────────────────────────────────┐ │
│ │ 店舗      │ 開始月 │ 年度    │ 操作                     │ │
│ │ 001 店舗A │ 4月    │ 2026年度│ [削除]                   │ │
│ │ 002 店舗B │ 1月    │ 2026年度│ [削除]                   │ │
│ └─────────────────────────────────────────────────────────┘ │
└─────────────────────────────────────────────────────────────┘
```
...
| 機能 | 説明 |
|------|------|
| 店舗選択プルダウン | デフォルト or 個別店舗を選択 |
| 複数店舗一括設定 | 選択した複数店舗に同じ設定を適用 |
| 店舗別設定一覧 | 個別設定がある店舗を表示 |
| 設定削除 | 個別設定を削除しデフォルトに戻す |
...
## 25. モーダルz-index階層仕様（2026-01-04追加）

### 25.1 概要

編集モーダルが親モーダルの後ろに表示される問題を解決するため、z-index階層を明確に定義。

### 25.2 z-index階層表

| 階層 | z-index | 対象モーダル | 説明 |
|------|---------|-------------|------|
| レベル1 | 1000 | 基本モーダル | 設定一覧、取り込み履歴等 |
| レベル2 | 1001 | リストモーダル | 項目一覧、カテゴリ一覧等 |
| レベル3 | 1002 | プレビューモーダル | データプレビュー |
| レベル4 | 1010 | 編集モーダル | 項目編集、店舗編集、客単価編集等 |

### 25.3 対象モーダル一覧

#### レベル4: 編集モーダル（z-index: 1010）

| モーダルID | 用途 |
|-----------|------|
| itemMappingEditModal | 項目マッピング編集 |
| storeEditModal | 店舗情報編集 |
| unitPriceEditModal | 客単価編集 |
| simUnitPriceEditModal | MQ試算用客単価編集 |
...
- 編集モーダルは常にリストモーダルより上位に表示される
- 編集モーダルを閉じると親モーダルが再度前面に表示される
- 複数の編集モーダルを同時に開くことは想定しない
...
テンプレート編集画面の「カテゴリ」列は、テキスト入力ではなくドロップダウン選択に変更。

#### 対象列の検出
```javascript
// 列ヘッダーに「カテゴリ」が含まれる場合
if (headerName.indexOf('カテゴリ') !== -1) {
    // ドロップダウンを表示
}
```

#### ドロップダウン選択肢（BUDGET_CATEGORY_MAP順）
| 順序 | カテゴリ名 |
|------|-----------|
| 1 | 売上高 |
| 2 | 売上原価 |
...
#### 機能仕様
- クリックで即座に新規テンプレートを作成（確認ダイアログなし）
- 履歴に保存すれば現在の編集内容は保持されるため、破棄確認は不要
...
function createNewTemplate() {
    ...
    currentTemplateHeaders = standardHeaders.slice();
    currentTemplateData = [/* 空の1行 */];
    // 保存名: 新規テンプレート_YYYY-MM-DD
}
```
...
### 2026-01-04
- セクション27: テンプレート編集機能強化を追加
- セクション25: モーダルz-index階層仕様を追加
- セクション24: インデックス混同防止ヘルパー関数を追加

### 2026-01-03
- セクション23: 会計年度設定機能を追加
- セクション22: 取り込みデータ履歴管理を追加
- セクション21: マスタデータ履歴管理を追加
- セクション20: MQ試算モーダルを追加
- セクション19: 予算試算機能（予算比較サマリー）を追加

### 2026-01-02
- セクション18: 設定インポート機能（店番・客単価・マッピング）を追加
- 金額表示の円単位対応
- 日次・月次表示の追加
